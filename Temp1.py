...
@author: finazzi
Go to python graph gallery
"""

import numpy as np
import matplotlib.pyplot as plt
"splt.hist(outcomes, bins = np.linspace(20,50,31)"
# Parameters
n_dice = 10
n_experiments = 1000
...
results = []
#Uniform distribution then sampling it
for _ in range(n_experiments):
    dice = np.random.randint(1, 7, n_dice)
    total = np.sum(dice)
    results.append(total)
#Plot distribution if you change the number of dice
# Sample mean 
